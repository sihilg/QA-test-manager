...
export function TestCases({ user, project, csrfToken, onBack, onLogout }: { user: User; project: Project; csrfToken: string; onBack: () => void; onLogout: () => void }) {
  ...
  return <div className="workspace">
    ...
    <section className="panel" aria-labelledby="case-list-title"><div className="section-heading"><div><h2 id="case-list-title">Casos ativos</h2><p>{cases.length} caso(s)</p></div><button type="button" className="tertiary" onClick={() => loadCases()}>Atualizar</button></div>{message && <p className={status === "error" ? "error" : "notice"} role="status">{message}</p>}{status === "loading" && <p className="empty-state">A carregar casos…</p>}{status === "error" && <button type="button" onClick={() => loadCases()}>Tentar novamente</button>}{status === "ready" && cases.length === 0 && <p className="empty-state">Ainda não existem casos para este projeto.</p>}{status === "ready" && cases.length > 0 && <div className="case-list">{cases.map((item) => <button type="button" className="case-row" key={item.id} onClick={() => openCase(item)}><strong>{item.case_number}</strong><span>{item.title}</span><span className={`result-badge ${item.final_result.toLowerCase()}`}>{item.final_result}</span></button>)}</div>}</section>
